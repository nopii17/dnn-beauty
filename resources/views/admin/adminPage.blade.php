...
<div class="bg-[#F9BAC9] py-8 text-center relative mb-[20rem] min-h-[400px]">
    <h1 class="text-5xl font-bold mb-14">Edit Profile as Admin</h1>
    <div
        class="bg-[#FAE1F5] shadow-md absolute left-1/2 -translate-x-1/2 flex flex-col items-center mx-auto px-5 py-5 w-2/3">
        <table class="bg-white w-full">
            <thead>
            <tr>
                <th class="border-black border-b px-4 py-5">Nama</th>
                <th class="border-black border-x border-b px-4 py-5">Username</th>
                <th class="border-black border-b px-4 py-5">Aksi</th>
            </tr>
            </thead>
            <tbody>
            @foreach($users as $user)
                <tr>
                    <td class="border-black border-r px-4 py-7">{{$user->name}}</td>
                    <td class="border-black border-r px-4 py-7">{{$user->username}}</td>
                    <td class="py-7">
                        <a href="{{ route('admin.edit', $user->id) }}"
                           class="bg-green-700 hover:bg-green-600 text-white font-bold py-2 px-4 mx-2 underline">Edit</a>
                        <form action="{{ route('admin.destroy', $user->id) }}" method="POST"
                              class="inline-block delete-form">
                            @csrf
                            @method('DELETE')
                            <button type="button"
                                    class="btn-delete bg-red-500 hover:bg-red-400 text-white font-bold py-2 px-4 mx-2 underline">
                                Delete
                            </button>
                        </form>
                    </td>
...
<footer class="bg-[#FDCEDF] px-24 py-8">
    <div class="grid grid-cols-6 gap-x-10">
        ...
        <div class="col-span-1 flex flex-col items-start gap-y-5">
            ...
            <a class="font-medium hover:underline hover:cursor-pointer" href="{{route('home')}}">Home</a>
